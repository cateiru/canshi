...
- 配信用 Route Handler
  ...
  - アクセス確認は `15` のミドルウェアに委ねる（本 PR では Access の検証ロジックを持たない）
- 上限の設定
...
- `15`（Cloudflare Access。配信エンドポイントの保護に必須。本 PR の着手時点で `15` が未完了の場合は先に `15` を完了させること）
...
- 配信エンドポイントから元データ・サムネイルを取得できる。ローカル環境では `15` のバイパス設定に従う
