...
import { vomitRecords } from "./vomit-records";

describe("vomit_records テーブル", () => {
  ...
  it("cat への参照つきで型安全に insert / select できる", async () => {
    ...
    const rows = await db.select().from(vomitRecords);

    expect(rows).toHaveLength(1);
    expect(rows[0].catId).toBe(cat.id);
    expect(rows[0].hasForeignObject).toBe(true);
    expect(rows[0].hasBlood).toBe(false);
  });
});
